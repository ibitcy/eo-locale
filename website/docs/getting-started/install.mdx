...
But by default, the JSC does not support libraries that manage internationalization.
To use `eo-locale`, you need to change JSC distribution and rebuild your project.

#### In React Native 0.60 or newer

Update `jscFlavor` in file `android/app/build.gradle` to `org.webkit:android-jsc-intl:+`.
...
```
(`X` and `Y` is a minor and patch version. Check on the [JSC build script repository](https://github.com/react-native-community/jsc-android-buildscripts))
...
You can find information on the [JSC build script repository](https://github.com/react-native-community/jsc-android-buildscripts).
